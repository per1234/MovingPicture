...
-consecutive leds only in positions - it must choose a start led and number of leds and then create the position from the available leds consecutive to that start led
  -pick start led, make sure it's not already in the other position
  -pick position size
  -add RGBLEDs above the start RGBLED until position size is reached or RGBLED already in other position is encountered
  -if the position hasn't been filled yet then add RGBLEDs below the start RBGLED until position size is reached or RGBLED 0 is reached
  -if the position hasn't been filled yet then add RGBLEDs starting from softPWM.Size is reached or RGBLED already in other position is encountered
...
-internal voltmeter high or low shutoff
...
-scripts - I can fill in extra memory with these and put different ones on each piece
  ...
  -multiple profiles cycle with power button - at startup: read current profile# from eeprom, increment, write profile# to eeprom
